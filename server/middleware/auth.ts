...
const publicRoutes = ['/dashboard', '/api/login', '/api/database', '/register', '/login', '/', '/forbidden', '/api/redis', '/api/redis2'];
...
// console.log('event.context.user:::>', event.context.user);
    ...
    if (!publicRoutes.includes(event.node.req.url)) {
        ...
        return sendRedirect(event, '/login', 302); // Redirect to '/login'
      }
